...
class ParamsSpec extends AnyFlatSpec with should.Matchers:
  "ModelParams" should "validate number of GPU layers" in:
    val withGpuLayers =
      (layers: GpuLayers) => ModelParams.parse(ModelParams(gpuLayers = layers))
  ...
  "Sampling" should "validate Dist params" in:
    ...
    val dry = Dry(penaltyLastN = Some(-1))
    withDist(Dist(dry = dry)) should be(Sampling.dryPenaltyLastNError)
    withDist(Dist(dry = Dry(penaltyLastN = Some(0)))) should be(
      Sampling.dryPenaltyLastNError
    )

    val penalty = Penalty(lastN = Some(-1))
    withDist(Dist(penalty = penalty)) should be(Sampling.penaltyLastNError)
    withDist(Dist(penalty = Penalty(lastN = Some(0)))) should be(
      Sampling.penaltyLastNError
    )
